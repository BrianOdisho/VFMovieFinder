...
import org.brianodisho.vfmoviefinder.discovermovies.DiscoverMoviesFragment;
...
public class MainActivity extends MvpActivity<MainView, MainPresenter> implements MainView, NavigationView.OnNavigationItemSelectedListener {
            ...
            public void onBackStackChanged() {
                if (getSupportFragmentManager().getBackStackEntryCount() == 0) {
                    if (_clearingBackStack) {
                        ...
                        navigationMenu.findItem(R.id.nav_discover_movies).setChecked(true);
                    }
                }
            }
        });
        showContentFragment(new DiscoverMoviesFragment(), false);
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.nav_discover_movies:
                presenter.onDiscoverMoviesSelected();
                break;
        }
        drawer.closeDrawer(GravityCompat.START);
        return true;
